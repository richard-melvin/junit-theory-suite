 ...
 * {@link com.github.radm.theories.TheorySuite} runner.
 */
...
public class ExampleTest {
	...
	@DataPoints("short")
	public static List<Year> years = IntStream.range(1995, 2001).boxed().map(Year::of).collect(Collectors.toList());
	...
	public void leapYearsHaveMoreDaysInFebruary(Year year) {
		if (year.isLeap()) {
			assertEquals(Month.FEBRUARY.maxLength(), year.atMonth(Month.FEBRUARY).lengthOfMonth());
		} else {
			assertEquals(Month.FEBRUARY.minLength(), year.atMonth(Month.FEBRUARY).lengthOfMonth());
		}
	}
	...
	/**
	 * For two arguments of the same types, the same set of values is used for
	 * each.
	 */
	@Theory
	 ...
	 * the currently running test. When the
	 * {@link com.github.radm.theories.TheorySuite} runner is used in a theory,
	 * this name contains the argument values used in the current test case.
	 */
	@Theory
